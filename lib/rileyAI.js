...
PERSONALITY & TONE:
- Professional but approachable, like talking to a trusted shop mechanic
- Direct and practical - you care about what actually works, not just specs
- Share real-world experience ("I've built hundreds of bikes with that combo...")
- Occasionally mention specific scenarios you've encountered
- Never condescending - respect that cyclists have varying experience levels
...
export class RileyAI {
  ...
  async askRiley(userQuestion, userSetup = null, analysisResults = null) {
    try {
      // Build context from user's current setup and analysis
...
Please respond as Riley, the expert bike mechanic:`;
      ...
      const rileyResponse = data.content[0].text;

      // Add to conversation history (keep last 6 exchanges)
      this.conversationHistory.push(
        { role: "user", content: userQuestion },
        { role: "assistant", content: rileyResponse }
      );
      ...
      const processedResponse = this.processAffiliateLinks(rileyResponse);
...
`;
  ...
  processAffiliateLinks(response) {
    // Enhanced affiliate link processing with more cycling components
    const affiliatePatterns = [
      // SRAM Eagle components
      { 
        pattern: /(SRAM (?:GX|X01|XX1|NX) Eagle (?:XG-1275|XG-1295|XG-1230))/gi,
        generateLink: (match) => `[${match}](https://www.jensonusa.com/search?q=${encodeURIComponent(match)}&utm_source=cranksmith&utm_medium=riley_ai)`
      },
      // Shimano MTB components
      { 
        pattern: /(Shimano (?:XT|XTR|SLX) (?:M8100|M9100|M7100))/gi,
        generateLink: (match) => `[${match}](https://www.competitivecyclist.com/search?q=${encodeURIComponent(match)}&utm_source=cranksmith&utm_medium=riley_ai)`
      },
      // Shimano Road components
      { 
        pattern: /(Shimano (?:105|Ultegra|Dura-Ace) (?:R7000|R8000|R9100))/gi,
        generateLink: (match) => `[${match}](https://www.competitivecyclist.com/search?q=${encodeURIComponent(match)}&utm_source=cranksmith&utm_medium=riley_ai)`
      },
      // Shimano GRX components
      { 
        pattern: /(Shimano GRX (?:RX600|RX810))/gi,
        generateLink: (match) => `[${match}](https://www.competitivecyclist.com/search?q=${encodeURIComponent(match)}&utm_source=cranksmith&utm_medium=riley_ai)`
      },
      // Generic pricing suggestions
      { 
        pattern: /(check (?:current )?pricing|latest prices|current deals|best price)/gi,
        generateLink: () => `[check current pricing](https://www.jensonusa.com/?utm_source=cranksmith&utm_medium=riley_ai)`
      },
      // Tool recommendations
      { 
        pattern: /(Park Tool|Pedro's|Shimano tool)/gi,
        generateLink: (match) => `[${match}](https://www.jensonusa.com/search?q=${encodeURIComponent(match)}&utm_source=cranksmith&utm_medium=riley_ai)`
      }
    ];
    ...
    affiliatePatterns.forEach(({ pattern, generateLink }) => {
      processedResponse = processedResponse.replace(pattern, (match) => {
        return generateLink(match);
      });
    });
